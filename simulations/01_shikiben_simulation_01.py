"""
Shikiben Experiment 01: Complete Dynamical Systems Verification & Plot Generation
Refactored to import core operators (P_gi) from the local `shikiben` package.
"""
...
def main():
    print("=" * 80)
    print(" Shikiben Experiment 01: Dynamical Simulation & Visual Verification")
    print("=" * 80)
    ...
    # 有害軸（x2軸：Ego Axis）を指定して shikiben の GeometricRectifier を構築
    # V_harmful は x2方向の単位ベクトル [0, 1]
    V_harmful = np.array([[0.0], [1.0]])
    rectifier = GeometricRectifier(V_harmful)
    P_gi = rectifier.P_gi  # shikiben パッケージの直交切断演算子 P_gi を直接参照

    ortho_check = np.zeros(t_steps)

    for i in range(1, t_steps):
        # (A) Shikiben 非適用（エゴ勾配による発散軌道）
    ...
    # Panel C: 感謝（O_kansha）による衝撃エネルギー変換ダイナミクス
    ax3 = axs[1, 0]
    ...
    plt.tight_layout()
    output_path = 'simulations/shikiben_v250_simulation_results.png'
    plt.savefig(output_path, dpi=300)
    
    print("[*] Simulation plot successfully generated using local `shikiben` package.")
