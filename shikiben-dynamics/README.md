# Shikiben (識扁) v2.4.0: Minimal Dissipation Mechanics & Virtue Phase Dynamics
...
3. **Jin (仁) — Multi-Agent Resonance Potential $\mathbf{S}_{\text{jin}}$**
   $$\mathbf{S}_{\text{jin}}(\mathbf{x}) = -\nabla \sum_{j} U_{\text{jin}}(\|\mathbf{x} - \mathbf{x}_j\|)$$
...
$$\dot{V}(\mathbf{x}) = \nabla \Psi_{\text{Taidou}}(\mathbf{x}) \cdot \dot{\mathbf{x}} = -\gamma \|\mathbf{g}_{\text{Taidou}}(\mathbf{x})\|^2 \le 0$$
...
| **Shikiben v2.4.0 (Virtue Phase)** | **Two-Stage Projection & Sliding** | **Minimal Steady ($P \le \epsilon$)** | **Minimal / Optimal** |

---

## 6. Quick Start & Simulation

To run the numerical validation trajectory:

```bash
cd simulations
pip install -r requirements.txt
python return_gradient.py
